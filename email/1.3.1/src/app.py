...
class Email(AppBase):
    ...
    def convert_msg_to_eml(self, msg_file_path):
        """
        Convertit un fichier .msg en .eml
        :param msg_file_path: Le chemin du fichier .msg à convertir
        :return: Un fichier .eml ou une erreur
        """
        try:
            # Ouvrir le fichier .msg avec extract_msg
            ...
            eml_msg['From'] = msg_sender
            eml_msg['Subject'] = msg_subject
            eml_msg['Date'] = msg_date
            ...
            eml_file_path = msg_file_path.replace(".msg", ".eml")
            with open(eml_file_path, "w") as eml_file:
                eml_file.write(eml_msg.as_string())

            return {
                "success": True,
                "message": f"Le fichier .msg a été converti en .eml avec succès : {eml_file_path}"
            }

        except Exception as e:
            return {
                "success": False,
                "reason": f"Erreur lors de la conversion du fichier .msg en .eml: {str(e)}"
            }
